...
r "sudo pihole-FTL sqlite3 /etc/pihole/gravity.db 'select count(*) from vw_gravity;'"
r "sudo pihole-FTL sqlite3 /etc/pihole/gravity.db 'select id, enabled, substr(address,1,70) from adlist order by id;'"
...
echo "| Kennzahl | Wert |"
echo "|---|---|"
echo "| Domains in gravity | $(w "sudo pihole-FTL sqlite3 /etc/pihole/gravity.db 'select count(*) from vw_gravity;'") |"
echo "| Blocklisten aktiv | $(w "sudo pihole-FTL sqlite3 /etc/pihole/gravity.db 'select count(*) from adlist where enabled=1;'") |"
echo "| Blocklisten abgeschaltet | $(w "sudo pihole-FTL sqlite3 /etc/pihole/gravity.db 'select count(*) from adlist where enabled=0;'") |"
...
echo ""
wert gravity_domains "$(sudo pihole-FTL sqlite3 /etc/pihole/gravity.db 'select count(*) from vw_gravity;' </dev/null 2>/dev/null)"
